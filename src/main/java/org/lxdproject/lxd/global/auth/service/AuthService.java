package org.lxdproject.lxd.global.auth.service;
...
import org.lxdproject.lxd.global.auth.converter.AuthConverter;
import org.lxdproject.lxd.global.auth.dto.AuthRequestDTO;
...
import org.lxdproject.lxd.global.auth.enums.VerificationType;
import org.lxdproject.lxd.global.config.properties.UrlProperties;
...
import org.lxdproject.lxd.global.infra.mail.MailService;
import org.lxdproject.lxd.global.infra.redis.RedisService;
...
public class AuthService {
    ...
    public void sendVerificationEmail(AuthRequestDTO.sendVerificationRequestDTO sendVerificationRequestDTO) {
        ...
        redisService.setVerificationToken(token, typeStr, sendVerificationRequestDTO.getEmail(), Duration.ofMinutes(5));

        // 이메일 중복 처리를 위한 보조키 저장
        redisService.setVerificationEmail(sendVerificationRequestDTO.getEmail(), token, Duration.ofMinutes(5));
    ...
    public void verifyEmailTokenAndRedirect(String token, HttpServletResponse response) {
        try {
            ...
            // 3. 원본 토큰 제거 -> 재사용 방지
            redisService.deleteVerificationToken(token);  // 리스트 토큰 삭제
            redisService.deleteVerificationEmail(email);  // 이메일 토큰 삭제
    ...
    public AuthResponseDTO.ReissueResponseDTO reissue(AuthRequestDTO.@Valid ReissueRequestDTO reissueRequestDTO) {
        ...
        String email = redisService.getRefreshTokenEmail(refreshToken);
        ...
        redisService.setRefreshToken(newRefreshToken, member.getEmail(), Duration.ofDays(7L));
